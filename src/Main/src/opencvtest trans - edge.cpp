...
int main(int argc, char** argv)
{
	...
	while (cap.read(matSrc))
	{
		...
		cv::resize(matSrcGray, matSrcGray, imgSize);

		// Transform to birdview
		cv::Mat matBirdview = ViewTransformer::toBirdview(matSrcGray);
		...
		// Blurr
		cv::Mat matBlurred;
		cv::GaussianBlur(matCut, matBlurred, cv::Size(3, 3), 0, 0, cv::BORDER_DEFAULT);
		...
		cv::Mat matColor;
		cv::cvtColor(matEdges, matColor, cv::COLOR_GRAY2BGR);
		...
		cv::Mat matFinal(matBinarized(cv::Rect(0, 0, matBinarized.size().width, matBinarized.size().height * (1 - ROAD_PART))));
